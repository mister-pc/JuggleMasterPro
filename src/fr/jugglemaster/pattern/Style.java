...
final public class Style {
	...
	static {
		...
		objS_DEFAULT_JUGGLE_STYLE = new Style(	Constants.strS_STRING_LOCAL_STYLE_DEFAULT,
												Constants.bytS_ENGINE_COORDONATES_NUMBER == 4	? new byte[] { 13, 0, 0, 0, 4, 0, 0, 0 }
																								: new byte[] { 13, 0, 0, 4, 0, 0 },
												(byte) 0,
		...
		@SuppressWarnings("unused") final byte[] bytLtetDeKranA = Constants.bytS_ENGINE_COORDONATES_NUMBER == 4
																													? new byte[] {	3, 11, 0, 0, 3, 11, 0,
																																0, 20, 20, 0, 0, 20,
																																20, 0, 0, 9, 14, 0, 0,
																																15, 17, 0, 0, 15, 17,
																																0, 0, 9, 14, 0, 0, 20,
		...
		objS_TET_DE_KRAN_JUGGLE_STYLE = new Style(	Constants.strS_ENGINE_TET_DE_KRAN_STYLE,
													bytLtetDeKranA,
													(byte) 0,
	...
	final private boolean[] getArmsBallsZOrders(boolean bolPnoReverse) {
		...
		if (bolPnoReverse) {
			System.arraycopy(	this.bolGarmsBallsZOrderA,
								0,
								bolLarmsBallsZOrderA,
								0,
								this.bytGstyleA.length / Constants.bytS_ENGINE_COORDONATES_NUMBER);
		} else {
			for (int intLarrayIndex = 0; intLarrayIndex < this.bytGstyleA.length / Constants.bytS_ENGINE_COORDONATES_NUMBER; ++intLarrayIndex) {
				bolLarmsBallsZOrderA[intLarrayIndex] = this.bolGarmsBallsZOrderA[this.bytGstyleA.length / Constants.bytS_ENGINE_COORDONATES_NUMBER
																					- intLarrayIndex - 1];
			}
		}
	...
	final public String toString(byte bytPfileFormat, boolean bolPdetailed, boolean bolPreverse, String strPreverse) {
		final StringBuilder objLstringBuilder = new StringBuilder(Constants.strS_BOOLEAN_LOCAL_CONTROL_A_A[Constants.bytS_BOOLEAN_LOCAL_STYLE][0].length()
																	+ 5 * (this.bytGstyleA.length + 1));

		// Write the style name :
		objLstringBuilder.append(bolPdetailed
									|| bytPfileFormat == Constants.bytS_EXTENSION_JM ? Strings.doConcat('%', bytPfileFormat == Constants.bytS_EXTENSION_JMP ? ' ' : null) : Strings.doConcat('#', Constants.strS_BOOLEAN_LOCAL_CONTROL_A_A[Constants.bytS_BOOLEAN_LOCAL_STYLE][0], " = "));
		...
		if (bolPdetailed) {
			...
			for (int intLxYZStyleIndex = 0; intLxYZStyleIndex < this.bytGstyleA.length; intLxYZStyleIndex += 2 * Constants.bytS_ENGINE_COORDONATES_NUMBER) {
				for (byte bytLxYZStyleDelta = 0; bytLxYZStyleDelta < 2 * Constants.bytS_ENGINE_COORDONATES_NUMBER; ++bytLxYZStyleDelta) {
					switch (bytLxYZStyleDelta % Constants.bytS_ENGINE_COORDONATES_NUMBER) {
						case 1:
							...
							//$FALL-THROUGH$
						case 0:
							intLxYZStyleLengthA[bytLxYZStyleDelta] = Math.max(	intLxYZStyleLengthA[bytLxYZStyleDelta],
																				(Byte.toString(bytLstyleA[intLxYZStyleIndex
																											+ bytLxYZStyleDelta])).length());
							break;
			...
			if (bytPfileFormat == Constants.bytS_EXTENSION_JMP && this.bolGzValues) {
				for (int intLzOrderIndex = 0; intLzOrderIndex < this.bolGbodyArmsZOrderA.length
					...
					intLzOrderLengthA[intLzOrderIndex
										% 2] = Math.max(intLzOrderLengthA[intLzOrderIndex % 2],
														Tools	.getZOrderString(bytLstyleA[intLzOrderIndex * Constants.bytS_ENGINE_COORDONATES_NUMBER
																							+ 2],
