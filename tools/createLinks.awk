...
function html(strP, intLstringLength, intLstringIndex, strLhtmlChar, strLhtml) {

    intLstringLength = length(strP);
    strLhtml = "";
    for (intLstringIndex = 1; intLstringIndex <= intLstringLength; intLstringIndex++) {
        strLhtmlChar = substr(strP, intLstringIndex, 1);
        if (strLhtmlChar == "à") {
            strLhtmlChar = "&agrave;";
        } else
...
BEGIN {
    ...
    # Print HTML header :
    printf("<?PHP\n");
    printf("    require('./juggleMasterPro.inc');\n");
    printf("    printHTMLHeader('links.css', true, false, true);\n");
