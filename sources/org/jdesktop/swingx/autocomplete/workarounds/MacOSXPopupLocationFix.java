 ...
 */
@SuppressWarnings("synthetic-access") public final class MacOSXPopupLocationFix {

	/**
	 * Private constructor so users use the more action-oriented {@link #install} method.
	 */
	private MacOSXPopupLocationFix(JComboBox objPjComboBox) {
		this.objGjComboBox = objPjComboBox;
		this.popupMenu = (JPopupMenu) objPjComboBox.getUI().getAccessibleChild(objPjComboBox, 0);

		this.popupMenu.addPopupMenuListener(this.listener);
	}

	void fixPopupLocation(JComponent popupComponent) {
		...
		final Point comboLocationOnScreen = this.objGjComboBox.getLocationOnScreen();
		final int comboHeight = this.objGjComboBox.getHeight();
		int popupY = comboLocationOnScreen.y + comboHeight;

		// ...unless the popup overflows the screen, in which case we put it
		// above the combobox
		final Rectangle screenBounds = new ScreenGeometry(this.objGjComboBox).getScreenBounds();
	...
	private final static class ScreenGeometry {
		...
		public Rectangle getScreenBounds() {
			...
			return new Rectangle(screenSize.x + screenInsets.left, screenSize.y + screenInsets.top, screenSize.width - screenInsets.left
		...
		private GraphicsConfiguration graphicsConfigurationForComponent(Component component) {
			...
			for (final GraphicsDevice element : gd) {
				...
				final GraphicsConfiguration defaultGraphicsConfiguration = element.getDefaultConfiguration();
	 ...
	 */
	public static MacOSXPopupLocationFix install(JComboBox objPjComboBox) {
		if (objPjComboBox == null) {
			throw new IllegalArgumentException();
		}
		return new MacOSXPopupLocationFix(objPjComboBox);
	}
	...
	private final JComboBox		objGjComboBox;
