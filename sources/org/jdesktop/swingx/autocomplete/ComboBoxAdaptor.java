...
public class ComboBoxAdaptor extends AbstractAutoCompleteAdaptor implements ActionListener {
	...
	@Override public Object getItem(int index) {
		return this.objGjComboBox.getItemAt(index);
	}

	@Override public int getItemCount() {
		return this.objGjComboBox.getItemCount();
	}

	@Override public Object getSelectedItem() {
		return this.objGjComboBox.getModel().getSelectedItem();
	}

	@Override public JTextComponent getTextComponent() {
		// returning the component of the combobox's editor
		return (JTextComponent) this.objGjComboBox.getEditor().getEditorComponent();
	}
	...
	@Override public void setSelectedItem(Object item) {
		...
		// other systems
		final Accessible a = this.objGjComboBox.getUI().getAccessibleChild(this.objGjComboBox, 0);
		...
		if (this.getItemCount() > 0 && a instanceof ComboPopup) {
			...
			if (rect == null) {
				throw new IllegalStateException("attempting to access index " + lastIndex + " for " + this.objGjComboBox);
			}
		...
		this.objGjComboBox.setSelectedItem(item);
	}
