...
public class AutoCompleteDecorator {
	...
	static class NonStrictBackspaceAction extends TextAction {
		...
		public NonStrictBackspaceAction(Action backspace1, Action selectionBackward1, AbstractAutoCompleteAdaptor adaptor1) {
			super("nonstrict-backspace");
			this.backspace = backspace1;
			this.selectionBackward = selectionBackward1;
	...
	static void removeKeyListener(Component c) {
		final KeyListener[] listeners = c.getKeyListeners();
	 ...
	 * if the combo box is not editable.
	 * <p>
	 * <b>Note:</b> the {@code AutoCompleteDecorator} will alter the state of the {@code JComboBox} to be editable. This can cause side effects with layouts and sizing. {@code JComboBox} caches the size, which differs depending on the component's editability. Therefore, if the component's size is accesed prior to being decorated and then the cached size is forced to be recalculated, the size of the component will change.
	 * <p>
	 * Because the size of the component can be altered (recalculated), the decorator does not attempt to set any sizes on the supplied {@code JComboBox}. Users that need to ensure sizes of supplied combos should take measures to set the size of the combo.
	 * 
	 * @param comboBox
	 *            a combo box
	...
	public static void decorate(final JComboBox comboBox, final ObjectToStringConverter stringConverter) {
		final boolean strictMatching = !comboBox.isEditable();
		// has to be editable
		...
		final AutoCompleteDocument document =
												AutoCompleteDecorator.createAutoCompleteDocument(	adaptor,
																									strictMatching,
																									stringConverter,
	...
	public static void decorate(JList list, JTextComponent textComponent, ObjectToStringConverter stringConverter) {
		final AbstractAutoCompleteAdaptor adaptor = new ListAdaptor(list, textComponent, stringConverter);
		final AutoCompleteDocument document =
												AutoCompleteDecorator.createAutoCompleteDocument(	adaptor,
																									true,
																									stringConverter,
	...
	public static void decorate(JTextComponent textComponent, AutoCompleteDocument document, final AbstractAutoCompleteAdaptor adaptor) {
		...
		final InputMap editorInputMap = textComponent.getInputMap();
		if (document.isStrictMatching()) {
			...
			editorInputMap.put(KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_DELETE, 0), AutoCompleteDecorator.errorFeedbackAction);
			editorInputMap.put(	KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_X, java.awt.event.InputEvent.CTRL_DOWN_MASK),
								AutoCompleteDecorator.errorFeedbackAction);
		} else {
			...
			editorActionMap.put("nonstrict-backspace", new NonStrictBackspaceAction(editorActionMap.get(DefaultEditorKit.deletePrevCharAction),
	 ...
	 *            <tt>true</tt>, if only given items should be allowed to be entered
	 */
	...
	public static void decorate(JTextComponent textComponent, List<?> items, boolean strictMatching, ObjectToStringConverter stringConverter) {
		final AbstractAutoCompleteAdaptor adaptor = new TextComponentAdaptor(textComponent, items);
		final AutoCompleteDocument document =
												AutoCompleteDecorator.createAutoCompleteDocument(	adaptor,
																									strictMatching,
																									stringConverter,
	...
	private static AutoCompleteDocument createAutoCompleteDocument(	AbstractAutoCompleteAdaptor adaptor,
	...
	private static void removeFocusListener(Component c) {
		final FocusListener[] listeners = c.getFocusListeners();
	...
	private static void removePropertyChangeListener(Component c) {
		final PropertyChangeListener[] listeners = c.getPropertyChangeListeners("editor");
